...
Set-Content -LiteralPath (Join-Path $certDir "az900-local-root-ca.pem") -Value $rootPem -Encoding ascii
[IO.File]::WriteAllBytes((Join-Path $certDir "az900-local-root-ca.cer"), $serverCert.Export([System.Security.Cryptography.X509Certificates.X509ContentType]::Cert))
Set-Content -LiteralPath (Join-Path $certDir "az900-local-server.pem") -Value $serverPem -Encoding ascii
Set-Content -LiteralPath (Join-Path $certDir "az900-local-server-key.pem") -Value $serverKeyPem -Encoding ascii

Write-Output "Created local HTTPS certificates in $certDir"
Write-Output "Install/trust certs\\az900-local-root-ca.cer on your iPhone before using the HTTPS phone URL."
